...
def genData(s):
    """takes a pair: <field>=<0xdata> and converts to a list with the name
    mapped using field_map and value converted to base 10 and appropriately
    scaled."""
    ...
    if f in [IDC, TNF, IL1]:  # current & frequency needs to be divided by 100
        v = v / 100.0
...
def convert_to_json(data):
    ...
    -- data: {01;FB;70|64:IDC=407;UL1=A01;TKK=2C;IL1=46D;SYS=4E28,0;\
    -- TNF=1383;UDC=B7D;PAC=16A6;PRL=2B;KT0=48C;SYS=4E28,0|1A5F}
    -- return:

    """
    # pull out the data elements into a list  and replace the code with the
    # json field name
    ev = [
        genData(s) for s in data[
            data.find(':') +
            1:data.find(
                '|',
                data.find(':'))].split(';')]
    ...
    # remove the trailing comma then close the braces
